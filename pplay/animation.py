...
class Animation(gameimage.GameImage):
    ...
    def __init__(self, image_file, total_frames, loop=True):
        ...
        # Final frame is inclusive (last valid index)
        self.set_sequence(0, self.total_frames - 1, self.loop)
    ...
    def set_sequence_time(self, initial_frame, final_frame,
        ...
        for _ in range(initial_frame, final_frame + 1):
            self.frame_duration.append(time_ms)
    ...
    def set_total_duration(self, time_ms):
        ...
        self.frame_duration = []
        for _ in range(0, self.total_frames):
    ...
    def draw(self):
        if(self.drawable):
            ...
            screen = window.Window.get_screen()
            if screen:
                screen.blit(self.image, self.rect, area=clip_rect)
